...
MAX_MOVIES = 7000 # Currently testing with 7000
...
class MovieProcessor:
    ...
    def extract_runtime(soup: BeautifulSoup) -> int:
        # Find the <p> tag with the class 'text-link text-footer'
        ...
        if runtime_tag:
            ...
            if match:
                ...
                # print(f"Extracted runtime: {runtime} minutes")
                return runtime
        # Raise an exception if no runtime is found
        raise RuntimeError("Runtime not found. The page structure may have changed.")
...
class LetterboxdScraper:
                ...
                for container in film_containers:
                    ...
                    if self.processor.is_whitelisted(film_title, release_year):
                        ...
                        if movie_identifier not in self.processor.added_movies:
                            ...
                            print(f"✅ {film_title} was added due to being whitelisted ({self.valid_movies_count + 1}/{MAX_MOVIES})")  # Updated reference
                    ...
                    if len(self.processor.film_data) < MAX_MOVIES:  # Only add to filtered films if under MAX_MOVIES
                        ...
                        print(f"✅ {film_title} was approved ({self.valid_movies_count + 1}/{MAX_MOVIES})")  # Updated reference
    ...
    def save_max_movies_2500_results(self):
        ...
        stats_path = os.path.join(BASE_DIR, f'rating_filtered_titles_stats.txt')
        ...
        for continent in CONTINENTS_COUNTRIES.keys():
            ...
            if continent_data:
                ...
                if top_data:
                    ...
                    stats_path = os.path.join(BASE_DIR, f'{continent.replace(" ", "_").lower()}_top_movies_stats.txt')
                    with open(stats_path, mode='w', encoding='utf-8') as file:
                        file.write(f"<strong>The Top {len(top_data)} Highest Rated Films from {continent}</strong>\n\n")
                        file.write(f"<strong>Last updated: {formatted_date}</strong>\n\n")
                        file.write("<strong>Film eligibility criteria:</strong>\n")
                        file.write("-- Must have a minimum of 1,000 reviews on Letterboxd.\n")
                        file.write("-- Cannot be a short film (minimum 40 minutes).\n")
    ...
    def save_results(self):
        ...
        with open(rejected_path, mode='a', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
    ...
    def save_mpaa_results(self):
        ...
        for rating in MPAA_RATINGS:
            ...
            if rating_data:
                ...
                # Save statistics for this rating
                stats_path = os.path.join(BASE_DIR, f'{rating.upper()}_top_movies_stats.txt')
                with open(stats_path, mode='w', encoding='utf-8') as file:
    ...
    def save_runtime_results(self):
        ...
        for category in RUNTIME_CATEGORIES.keys():
            ...
            if category_data:
                ...
                stats_path = os.path.join(BASE_DIR, f'{category}_top_movies_stats.txt')
                with open(stats_path, mode='w', encoding='utf-8') as file:
    ...
    def save_unknown_continent_films(self):
        ...
        if self.unknown_continent_films:
            ...
            print(f"Saved {len(self.unknown_continent_films)} films from unknown continents to {output_path}.")
